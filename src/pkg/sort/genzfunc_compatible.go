...
// to generate zfuncversion_compatible.go.
//
// It copies sort.go to zfuncversion_compatible.go, only retaining funcs which
...
func main() {
	...
	src = regexp.MustCompile(`(?m)^func (\w+)`).ReplaceAll(
...
`)
	...
	const target = "zfuncversion_compatible.go"
